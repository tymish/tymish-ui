...
export class AddEmployeeComponent implements OnInit {
  ...
  public form = new FormGroup({
    givenName: new FormControl('', [Validators.required, Validators.maxLength(50)]),
    familyName: new FormControl('', [Validators.required, Validators.maxLength(50)]),
    email: new FormControl('', [Validators.required, Validators.pattern('^([a-zA-Z0-9_\\-\\.]+)@([a-zA-Z0-9_\\-\\.]+)\\.([a-zA-Z]{2,5})$')]),
    hourlyPay: new FormControl('', [Validators.required, Validators.min(0), Validators.max(100)])
  });
